...
def pull_messages(client, project_name, sub_name):
    ...
    if receivedMessages is not None:
    	print "received messages has data\n"
        ack_ids = []
...
def write_to_bq(pubsub, sub_name, bigquery):
    ...
    while count < count_max:
        while len(traffic_segments) < CHUNK:
            traffic_messages = pull_messages(pubsub, PROJECT_ID, sub_name)
            if traffic_messages:
                for res in traffic_messages:
                    # First do some massaging of the raw data
                    try:
                    	res = res.replace("'",'"')
                    	res = res.replace('u"','"')
                        
                        traffic_seg = json.loads(res)
                    except Exception, bqe:
                        print bqe
                    mtraffic_seg = utils.cleanup(traffic_seg)
                    traffic_segments.append(mtraffic_seg)
            else:
                # pause before checking again
                print 'sleeping...'
                time.sleep(WAIT)
        print traffic_segments
        response = utils.bq_data_insert(bigquery, PROJECT_ID, os.environ['BQ_DATASET'],
                             os.environ['BQ_TABLE'], traffic_segments)
        print "BQ write : " + str(response)
        traffic_segments = []
        count += 1
        if count % 25 == 0:
