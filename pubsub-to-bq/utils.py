    ...
    if isinstance(data, dict):
        ...
        for k, v in data.items():
            if (k == 'street' or k == 'street_heading' or k == 'speed' or k == 'comments' or k == 'direction' or k == 'segment_id' or k == 'length'):
                ...
                newdict['end_lat'] = v
            elif k == 'start_latitude':
                newdict['start_lat'] = v
            elif k == 'end_longitude':
                newdict['end_long'] = v
            elif k == 'start_longitude':
                newdict['start_long'] = v
            elif k == 'time' and v:
                newdict['last_updated'] = str(dateutil.parser.parse(v))
            else:
                pass
        return newdict
...
def bq_data_insert(bigquery, project_id, dataset, table, traffic_segments):
